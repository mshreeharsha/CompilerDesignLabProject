...
Exp: OPEN_BRACK Exp CLOSE_BRACK {$$=malloc(strlen($2)+1); strcpy($$,$2);}
     | Exp PLUS Exp {
        int ans=checkCompartibility($1,$3);
        if(ans==-1){
            char const errorMessage[100]="Incompatible Data Types!!";
            int a=yyerror(errorMessage);
            return -1;
        }
        else{
            $$=malloc(strlen(datatypesArray[ans])+1);
            strcpy($$,datatypesArray[ans]);
        }
     }
     | Exp MINUS Exp {
        int ans=checkCompartibility($1,$3);
